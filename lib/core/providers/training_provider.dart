...
class TrainingProvider with ChangeNotifier {
  ...
  Future<void> _saveProgressToFirebase(
    ...
    try {
      ...
      // Don't throw error for Firebase sync - continue with local storage
      if (kDebugMode) {
        print('Failed to sync progress to Firebase: $e');
      }
    }
  }
    ...
    try {
      ...
      if (user != null) {
        ...
        if (result['success'] == true && result['data'] != null) {
          final firebaseData = result['data'] as Map<String, dynamic>;
          final prefs = await SharedPreferences.getInstance();
          final localDataString = prefs.getString('training_progress');
          ...
          try {
            firebaseTimestamp =
                DateTime.parse(firebaseData['lastUpdated'] ?? '');
          } catch (e) {
          ...
          if (localDataString != null) {
            try {
              final localData =
                  jsonDecode(localDataString) as Map<String, dynamic>;
              localTimestamp = DateTime.parse(localData['lastUpdated'] ?? '');
            } catch (e) {
              localTimestamp = null;
            }
          }

          // Use Firebase data if it's newer or if there's no local data
          bool useFirebaseData = false;
          if (localTimestamp == null) {
            useFirebaseData = true;
          } else if (firebaseTimestamp != null &&
              firebaseTimestamp.isAfter(localTimestamp)) {
            useFirebaseData = true;
          }

          if (useFirebaseData) {
            final modulesList = firebaseData['modules'] as List<dynamic>?;

            if (modulesList != null && _modules.isNotEmpty) {
              // Update existing modules with Firebase progress
              for (final savedModuleJson in modulesList) {
                final savedModule = TrainingModule.fromJson(savedModuleJson);
                final moduleIndex =
                    _modules.indexWhere((m) => m.id == savedModule.id);
              ...
              // Restore current module
              _currentModuleId = firebaseData['currentModuleId'];

              // Save to local storage to keep them in sync
              await prefs.setString(
                  'training_progress', jsonEncode(firebaseData));
      ...
      // Silently fail for Firebase loading - don't show error to user
      if (kDebugMode) {
        print('Failed to load progress from Firebase: $e');
      }
    }
  }
