...
You can execute the `GetReviewsForProduct` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getReviewsForProduct(vars: GetReviewsForProductVariables): QueryPromise<GetReviewsForProductData, GetReviewsForProductVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
getReviewsForProduct(dc: DataConnect, vars: GetReviewsForProductVariables): QueryPromise<GetReviewsForProductData, GetReviewsForProductVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getReviewsForProductRef:
```typescript
const name = getReviewsForProductRef.operationName;
console.log(name);
```

### Variables
The `GetReviewsForProduct` query requires an argument of type `GetReviewsForProductVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `GetReviewsForProduct` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetReviewsForProductData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetReviewsForProduct`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getReviewsForProduct, GetReviewsForProductVariables } from '@dataconnect/generated';

// The `GetReviewsForProduct` query requires an argument of type `GetReviewsForProductVariables`:
const getReviewsForProductVars: GetReviewsForProductVariables = {
  productId: ..., 
};

// Call the `getReviewsForProduct()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getReviewsForProduct(getReviewsForProductVars);
...
// Or, you can use the `Promise` API.
getReviewsForProduct(getReviewsForProductVars).then((response) => {
  const data = response.data;
  console.log(data.reviews);
});
```

### Using `GetReviewsForProduct`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getReviewsForProductRef, GetReviewsForProductVariables } from '@dataconnect/generated';
...
// Call the `getReviewsForProductRef()` function to get a reference to the query.
const ref = getReviewsForProductRef(getReviewsForProductVars);
// Variables can be defined inline as well.
const ref = getReviewsForProductRef({ productId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getReviewsForProductRef(dataConnect, getReviewsForProductVars);
...
// Variables can be defined inline as well.
const { data } = await updateProduct({ id: ..., name: ..., price: ..., });
...
// Variables can be defined inline as well.
const ref = updateProductRef({ id: ..., name: ..., price: ..., });
